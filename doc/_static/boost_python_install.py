...
def compile_boost():
    ...
    base_cmd_line = "b2 -j{cpus} {silent} {debug_config} --with-python --build-dir={{build-dir}}".format(
        cpus=cpus, silent=silent_args, debug_config=debug_config_args
    )
                ...
                for link in links:
                    ...
                    for runtime_link in runtime_links:
                        ...
                        if link == "shared" and runtime_link == "static":
                            print(
                                "--> Skipping invalid compile option link=shared runtime_link=static <--"
                            )
                            continue
                        for threading in threadings:
                            kwargs["threading"], threading_dir = to_name_and_dir(
                                threading
                            )
